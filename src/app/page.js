...
const RUOLI = [
  { key:'elite',    label:'Élite Coach',       counted:true,  color:'#1a6b3c' },
  { key:'head',     label:'Head Coach',         counted:true,  color:'#1565c0' },
  ...
  { key:'physical', label:'Physical Trainer',   counted:true,  color:'#f57c00' },
  { key:'travel',   label:'Travelling Coach',   counted:false, color:'#795548' },
  { key:'video',    label:'Video/Fisio/Mental', counted:false, color:'#6a1b9a' },
  { key:'extra',    label:'Extra Program',      counted:false, color:'#9e9e9e' },
]
...
async function sbUpsert(table, data) {
  const r = await fetch(`${SUPABASE_URL}/rest/v1/${table}`, {
    method:'POST', headers:{...H, Prefer:'return=representation,resolution=merge-duplicates'}, body:JSON.stringify(data)
  })
...
async function dbLoadWeeks() {
  const [settimane, maestri] = await Promise.all([
    sbGet('planner_settimane','select=*&order=num'),
    sbGet('planner_maestri','select=*'),
  ])
  const result = {}
  SECTIONS.forEach(s => { result[s.id] = [] })
  settimane.forEach(s => {
    if (!result[s.sezione]) result[s.sezione] = []
    const rows = maestri.filter(m=>m.settimana_id===s.id).map(m=>({
      id:m.id, nome:m.nome, ruolo:m.ruolo, impegno:m.impegno||'',
    ...
    result[s.sezione].push({ id:s.id, num:s.num, note:s.note||'', rows })
  })
  return result
}
async function dbSaveWeek(sezione, week) {
  await sbUpsert('planner_settimane', { id:week.id, sezione, num:week.num, note:week.note||'', anno:2026 })
}
async function dbSaveRow(settimana_id, row) {
  await sbUpsert('planner_maestri', {
    id:row.id, settimana_id, nome:row.nome, ruolo:row.ruolo,
    impegno:row.impegno||'', giorni:JSON.stringify(row.giorni),
    coach_ref_id: row.coachId?parseInt(row.coachId):null,
  })
}
async function dbDeleteWeek(weekId) {
  await sbDelete('planner_maestri',`settimana_id=eq.${weekId}`)
  await sbDelete('planner_settimane',`id=eq.${weekId}`)
}
async function dbDeleteRow(rowId) {
  await sbDelete('planner_maestri',`id=eq.${rowId}`)
}

// ── calcCounters ──────────────────────────────────────────────────────
function calcCounters(rows) {
  const counted = ['elite','head','coach','asst','tiro','physical']
  const centro    = DAYS.map((_,di)=>rows.filter(r=>counted.includes(r.ruolo)&&r.giorni[di]&&!r.impegno).length)
  ...
  return { centro, torneo, travel, extra, byRole }
}
...
function DashboardPage({ weeks, coaches }) {
  ...
  const globalByMonth = MONTHS.map(m=>{
    ...
    const allRows=(weeks[sid]||[]).flatMap(w=>w.rows)
    const vals=DAYS.map((_,di)=>allRows.filter(r=>['elite','head','coach','asst','tiro','physical'].includes(r.ruolo)&&r.giorni[di]&&!r.impegno).length)
    return {month:m, avg:vals.reduce((a,b)=>a+b,0)/7}
  })
  ...
  const stats = selCoach ? getStats(selCoach) : null
  return (
    <div style={{padding:16,maxWidth:1100,margin:'0 auto'}}>
      <h1 style={{fontSize:20,fontWeight:800,color:G.text,margin:'0 0 4px'}}>📊 Riepilogo Annuale 2026</h1>
      <p style={{fontSize:12,color:G.textMid,margin:'0 0 16px'}}>Presenze per mese e per coach</p>
...
function CalendarioPage({ weeks, coaches, tornei }) {
  ...
  const startDow=(new Date(2026,calMonth,1).getDay()+6)%7
      ...
      <div style={{background:'#fff',borderRadius:12,border:`1px solid ${G.border}`,overflow:'hidden'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)'}}>
          {cells.map((day,i)=>{
            ...
            const fuori=m.filter(x=>x.impegno)
            const travel=m.filter(x=>x.ruolo==='travel')
...
function CoachPage({ coaches, weeks, onUpdateCoach, onAddCoach }) {
  ...
  async function changeRuolo(coach, ruolo) {
    setSaving(coach.id)
    const ok = await sbPatch('coach',`id=eq.${coach.id}`,{ruolo})
  ...
  function getStats(nome) {
    ...
    Object.values(weeks).forEach(wks=>wks.forEach(w=>{
      ...
      else if(row.impegno==='stage')ss++; else if(row.ruolo==='extra')se++
...
function WeekBlock({ week, coaches, tornei, sectionName, onUpdate, onDelete, onAddRow, onDeleteRow, onCopyNext }) {
  ...
  const { centro, torneo, travel, extra, byRole } = calcCounters(week.rows)
      ...
      <div style={{background:G.greenLight,borderBottom:`1px solid ${G.border}`,padding:'10px 14px'}}>
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          ...
          <div style={{width:42,height:42,borderRadius:10,background:G.green,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,overflow:'hidden'}}>
            <input type="number" min={1} max={31} value={editNum}
              onChange={e=>setEditNum(e.target.value)}
              onBlur={()=>{ const n=parseInt(editNum); if(n&&n!==week.num) onUpdate({...week,num:n}); else setEditNum(String(week.num)) }}
              onKeyDown={e=>e.key==='Enter'&&e.target.blur()}
              style={{width:42,height:42,background:'transparent',border:'none',outline:'none',textAlign:'center',fontSize:18,fontWeight:800,color:'#fff',fontFamily:'inherit',cursor:'text'}}
        ...
        <div style={{display:'flex',gap:4,marginTop:10,overflowX:'auto',paddingBottom:2}}>
          ...
          <div style={{flexShrink:0,display:'flex',gap:4,marginLeft:8}}>
            ...
            <button onClick={onCopyNext} title="Copia nella settimana successiva" style={{background:'#fff',border:`1.5px solid ${G.blue}`,color:G.blue,borderRadius:8,padding:'4px 8px',fontFamily:'inherit',fontSize:12,fontWeight:700,cursor:'pointer',whiteSpace:'nowrap'}}>📋 +7</button>
                      ...
                      <div key={row.id} style={{padding:'5px 8px',borderBottom:`1px solid #f0f5f0`,background:row.impegno?impegnoInfo?.color+'08':ri%2===0?'#fff':'#fafcfa'}}>
                        <div style={{display:'flex',alignItems:'center',gap:4,marginBottom:4}}>
                          ...
                          <button onClick={()=>onDeleteRow(row.id)}
                            style={{background:'none',border:'none',cursor:'pointer',color:'#ddd',fontSize:12,padding:'0 2px',flexShrink:0,lineHeight:1}}
...
export default function Home() {
  ...
  const loadAll = useCallback(async()=>{
    ...
    try {
      const [w,c,t] = await Promise.all([
        dbLoadWeeks(),
        sbGet('coach','select=*&order=nome'),
        sbGet('tornei','select=*&order=data'),
      ])
      setWeeks(w); setCoaches(c); setTornei(t); setSyncStatus('ok')
    ...
    try {
      ...
      ws.onmessage=(e)=>{
        ...
        if(['INSERT','UPDATE','DELETE'].includes(msg.event)){
          if(msg.topic?.includes('tornei')) sbGet('tornei','select=*&order=data').then(setTornei)
          if(msg.topic?.includes('coach')) sbGet('coach','select=*&order=nome').then(setCoaches)
        }
      }
      wsRef.current=ws
      return()=>ws.close()
    } catch(e){ const iv=setInterval(()=>sbGet('tornei','select=*&order=data').then(setTornei),30000); return()=>clearInterval(iv) }
  },[loggedIn,loadAll])
  ...
  async function addWeekFn(num, note) {
    const week={id:uid(),num,note:note||'',rows:[]}
    setWeeks(prev=>({...prev,[active]:[...(prev[active]||[]),week].sort((a,b)=>a.num-b.num)}))
    await dbSaveWeek(active,week); doFlash()
  }
  ...
  async function addRow(weekId,row) {
    ...
    await dbSaveRow(weekId,row); doFlash()
  }
  ...
  async function deleteRow(weekId,rowId) {
    ...
    await dbDeleteRow(rowId); doFlash()
  }
  ...
  async function copyWeekToNext(week) {
    ...
    if(existing){
      ...
      if(toAdd.length===0){alert(`La settimana ${nextNum} ha già gli stessi maestri.`);return}
  ...
  async function copyMonthToNext() {
    const currentIdx=MONTHS.indexOf(activeSection?.name)
    if(currentIdx<0||currentIdx>=11){alert('Seleziona un mese valido (non Dicembre).');return}
    const nextName=MONTHS[currentIdx+1]
    const nextSec=SECTIONS.find(s=>s.name===nextName)
    if(!nextSec) return
    if(wks.length===0){alert('Nessuna settimana da copiare.');return}
    if(!window.confirm(`Copia tutte le settimane di ${activeSection.name} in ${nextName}?`)) return
    ...
    doFlash(); alert(`✅ Copiate ${added} settimane in ${nextName}!`)
  }

  async function exportCSV() {
    try {
      const [settimane,maestri,coachList]=await Promise.all([
        sbGet('planner_settimane','select=*&order=sezione,num'),
        sbGet('planner_maestri','select=*'),
        sbGet('coach','select=*&order=nome'),
      ])
      function toCSV(rows) {
  ...
  function confirmAddRow(){
    ...
    if(!nome) return
    addRow(modalRow,{id:uid(),nome,ruolo,giorni:newGiorni,impegno:newImpegno,coachId:newCoachId||null})
    setModalRow(null)
  }

  function confirmAddWeek(){
    const num=parseInt(newWeekNum)||((wks.slice(-1)[0]?.num||0)+7)
    addWeekFn(num,newWeekNote.trim())
    ...
    <div style={{minHeight:'100vh',background:G.bg}}>
      ...
      <nav style={{background:'#fff',borderBottom:`1px solid ${G.border}`,padding:'0 14px',height:54,display:'flex',alignItems:'center',gap:8,position:'sticky',top:0,zIndex:200,boxShadow:'0 1px 6px rgba(26,107,60,0.07)'}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:5,flexShrink:0}}>
          ...
          {trash.length>0&&(
            <button onClick={()=>setShowTrash(true)} style={{background:'#fff8f0',border:`1px solid #fecaca`,borderRadius:8,padding:'4px 8px',cursor:'pointer',fontSize:11,color:G.red,fontWeight:700}}>🗑️ {trash.length}</button>
          )}
          <button onClick={exportCSV} title="Scarica backup CSV" style={{background:'none',border:`1px solid ${G.border}`,borderRadius:8,padding:'4px 8px',cursor:'pointer',fontSize:11,color:G.textMid}}>💾</button>
          <button onClick={doLogout} style={{background:'none',border:`1px solid ${G.border}`,borderRadius:8,padding:'4px 8px',cursor:'pointer',fontSize:11,color:G.textMid}}>🚪</button>
        </div>
      </nav>
        ...
        <div style={{padding:14,maxWidth:1400,margin:'0 auto'}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14,gap:10,flexWrap:'wrap'}}>
            <div>
              ...
              <p style={{fontSize:12,color:G.textMid,margin:'2px 0 0'}}>{wks.length} sett. · {wks.reduce((s,w)=>s+w.rows.length,0)} maestri · ☁️ cloud</p>
            </div>
            <div style={{display:'flex',gap:8}}>
              <button onClick={copyMonthToNext} style={{background:'#fff',border:`1.5px solid ${G.blue}`,color:G.blue,borderRadius:10,padding:'9px 14px',fontFamily:'inherit',fontSize:12,fontWeight:700,cursor:'pointer',whiteSpace:'nowrap'}}>📋 Copia mese →</button>
